...
# data2 = np.loadtxt(fname='Decoupled_Morse_harmonic_FC_2DES_0.dat')
...
for i in range(Num_2DES_frames):
    ...
    max_intensity_each = max(data[:,2])
    intensity_row_index,intensity_column_index = (np.where(data==max_intensity_each))
    energy_max = data[int(intensity_row_index),1]
    # intensity_max = data[int(intensity_row_index),1]

    #print('max_value: ',max_value) #uncomment to verify max values if needed
    max_list[i,0] = max_intensity_each
    max_list[i,1] = energy_max
    # max_list[i,0] = time_counter
    time_counter = time_counter+Delay_time_step
print(max_list.shape)
np.savetxt('2DES_max_Cumulant_GB_int_vs_ev.txt',max_list)
